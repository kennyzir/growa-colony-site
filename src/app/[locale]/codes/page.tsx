...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  if (locale === "es") {
    return {
      title: `Códigos de Grow A Colony (${monthLabel})`,
      description: `Estado actual de los códigos de recompensa de Grow A Colony, dónde buscar nuevos códigos y cómo canjearlos en el juego.`,
...
const sourceLinks = [
  {
    ...
    href: "https://www.roblox.com/games/103154273080028/Grow-A-Colony",
...
const redemptionSteps = [
  {
    name: "Open Grow A Colony from the official Roblox page",
    text: "Launch the game from roblox.com/games/103154273080028/Grow-A-Colony to avoid fake copies. Let it fully load."
  },
  {
    name: "Look for a rewards/redemption panel in-game",
    text: "Roblox colony sims commonly put code redemption somewhere in the in-game shop or settings. The exact panel is confirmed in the current build."
  },
  {
...
const refreshHistory = [
  {
    ...
    note: "As of this refresh no Grow A Colony code had an official, dated in-game confirmation we can point to. The active list stays empty rather than showing guessed or stale strings."
  }
];
...
export default async function CodesPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const links = [
    { href: `${prefix}/how-to-play`, title: isEs ? "Cómo jugar" : "How to Play", desc: isEs ? "El bucle de construcción de colonia y primeros pasos." : "The colony-building loop and first steps." },
    { href: `${prefix}/release-date`, title: isEs ? "Fecha de lanzamiento" : "Release Date", desc: isEs ? "Cuándo salió Grow A Colony." : "When Grow A Colony launched." },
    { href: `${prefix}/updates`, title: isEs ? "Actualizaciones" : "Updates", desc: isEs ? "Novidades del juego." : "What's new in the game." },
    { href: `/wiki`, title: isEs ? "Explorador Wiki" : "Wiki Explorer", desc: isEs ? "Todo sobre Grow A Colony en un lugar." : "Everything about Grow A Colony in one place." }
  ];
  ...
  const T = {
    ...
    redeemCopy: isEs ? "Los pasos de canje dependen del juego actual. Abre Grow A Colony, busca un panel de recompensas en la tienda o ajustes, e introduce el código. No confirmamos un mecanismo específico hasta verificarlo en el build actual." : "Redemption steps depend on the current build. Open Grow A Colony, look for a rewards panel in the shop or settings, and enter the code. We don't confirm a specific mechanism until we verify it in the current build.",
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-10">
      ...
      <section className="mt-10">
        ...
        <HowToJsonLd name={isEs ? "Canjear un código de Grow A Colony" : "Redeem a Grow A Colony code"} description={isEs ? "Cómo canjear un código de recompensa en Grow A Colony." : "How to redeem a reward code in Grow A Colony."} steps={[
